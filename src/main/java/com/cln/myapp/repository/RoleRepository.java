...
public interface RoleRepository extends JpaRepository<Role, Long> {
    ...
    @Query(value = "select distinct role from Role role left join fetch role.profils",
        countQuery = "select count(distinct role) from Role role")
    Page<Role> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct role from Role role left join fetch role.profils")
    List<Role> findAllWithEagerRelationships();

    @Query("select role from Role role left join fetch role.profils where role.id =:id")
