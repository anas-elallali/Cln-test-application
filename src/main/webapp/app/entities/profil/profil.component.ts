...
export class ProfilComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.profilService
        ...
        .subscribe(
          (res: HttpResponse<IProfil[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInProfils(): void {
    this.eventSubscriber = this.eventManager.subscribe('profilListModification', () => this.loadPage());
  }
