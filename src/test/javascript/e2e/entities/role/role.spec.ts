...
describe('Role e2e test', () => {
  ...
  it('should load Roles', async () => {
    ...
    expect(await roleComponentsPage.getTitle()).to.eq('clnTestApp.role.home.title');
  ...
  it('should load create Role page', async () => {
    ...
    expect(await roleUpdatePage.getPageTitle()).to.eq('clnTestApp.role.home.createOrEditLabel');
  ...
  it('should create and save Roles', async () => {
    ...
    await promise.all([
      ...
      // roleUpdatePage.profilsSelectLastOption(),
    ]);

    expect(await roleUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
  ...
  it('should delete last Role', async () => {
    ...
    expect(await roleDeleteDialog.getDialogTitle()).to.eq('clnTestApp.role.delete.question');
